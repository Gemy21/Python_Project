...
class Database:
    ...
    def init_database(self):
        ...
        # جدول تفاصيل النقلات (العدد والوزن الكلي)
    ...
    def get_agriculture_transfers(self):
        ...
        cursor.execute('SELECT id, shipment_name, seller_name, item_name, unit_price, weight, count, equipment, transfer_type, created_at FROM agriculture_transfers ORDER BY created_at DESC')
    ...
    def get_transfer_by_id(self, transfer_id):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM agriculture_transfers WHERE id = ?", (transfer_id,))
        result = cursor.fetchone()
        conn.close()
        return result
    ...
    def get_all_client_invoices(self):
        """جلب جميع فواتير العملاء (لكل العملاء)"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            SELECT id, owner_name, nolon, commission, mashal, rent, cash, invoice_date, net_amount, final_total 
    ...
    def get_all_collections(self):
        """جلب كل التحصيلات (المعاملات المدفوعة والسماح من البائعين)"""
        conn = self.get_connection()
        cursor = conn.cursor()
        
        cursor.execute('''
            SELECT st.id, sa.seller_name, st.amount, st.date, st.note, st.status
            FROM seller_transactions st
            JOIN sellers_accounts sa ON st.seller_id = sa.id
            WHERE st.status IN ('مدفوع', 'سماح')
